...
def process_era5_data(path_to_data, VARIABLE, date):
    ...
    Outputs:
        PNG plots of ensemble mean and standard deviation for each time step and height.
    """
    ...
    try:
        ...
        for i, timestamp in enumerate(ds_flipped.time.values):
            ...
            # Loop through hybrid levels
            for j, hgt in enumerate(ds_flipped.hybrid.values):
                print(f"Processing time {i} (UTC {UTC_hour}), ML index {j}")

                t_slice = ds_flipped[VARIABLE].sel(time=timestamp, hybrid=hgt)
                ...
                ens_mean = ensemble_mean.interp(x=new_x, y=new_y)
                ens_std = ensemble_std.interp(x=new_x, y=new_y)

                # Determine plotting levels
                ...
                def plot_ens(data, filename, levels, label):
                    ...
                    data.plot(levels=levels, cmap='jet', extend='both',
                              cbar_kwargs={'label': label, 'shrink': 0.8})
                    plt.xlabel("Longitude")
                    plt.ylabel("Latitude")
                    plt.title(f"{label} ML at {j} (UTC {UTC_hour})")
                ...
                plot_sd(
                    ens_std,
                    f"SD_{VARIABLE}_{YYYY}{MM}{DD}_UTC_{i}_ML_{j}.png",
                    vmin=0,
                    vmax=3,
                ...
                plot_ens(
                    ens_mean,
                    f"EnsMEAN_{VARIABLE}_{YYYY}{MM}{DD}_UTC_{i}_ML_{j}.png",
                    levels=levels,
                    label='Ensemble Mean'
                )

                print(f"Saved plots for time {i}, ML {j}")
            # Loop through hybrid levels
        # Loop through time steps
