...
import numpy as np
import zarr
from process_data_CARRA2 import process_carra2_data
from config import *
...
SCRPDIR = "/home/swe4281/repository/CARRA_QC2025/Uncertainty_Quantification/netCDFtozarr"  # directory of conversion script
...
elif MM in ["03", "04", "09", "10"]:
    INPUT1 = f"/ec/res4/scratch/swe4281/DDPM_DATA/KESP/NCFILES/"
else:
    print("Month", MM, "not processed.", flush=True)
    exit()
...
for DD in range(DTSTR, DTEND+1):  # loop over days
    ...
    for HH in ['00', '06', '12', '18']:  # loop over hours
        ...
        for param in params:  # loop over parameters
            ...
            print(f"\nCARRA2 variable to be processed: {param} ({FA_name[param]}), for {timestamp}", flush=True)


            try:

                # process CARRA2 data (for param)
                ds_param = process_carra2_data(INPUT1,
                                                param,
                                                YY, MM, DDN, HH)
