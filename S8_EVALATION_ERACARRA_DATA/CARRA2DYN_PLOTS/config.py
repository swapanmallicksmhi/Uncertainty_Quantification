...
def plot_dyn(stat_name, stat_data, output_file, timestamp, levels):
    fig, ax = plt.subplots(figsize=(8, 6.5))
    stat_data.plot(
        ax=ax, levels=levels, cmap='jet', extend='both',
        add_colorbar=True, cbar_kwargs={'label': '', 'shrink': 0.8}
...
def plot_dynC(stat_name, stat_data, output_file, timestamp, levels):
    fig, ax = plt.subplots(figsize=(8, 6.5))
    stat_data.plot(
        ax=ax, levels=levels, cmap='jet',
        add_colorbar=False,   # turn off colorbar entirely
        add_labels=False
    )
    ...
    # Optional: also clear tick labels if they sneak in
    ax.set_xticklabels([])
    ax.set_yticklabels([])

    # Force the domain to be square
